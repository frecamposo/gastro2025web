...
#mio
@router.get("/usuario/deshabilitar/{id_usuario}")
@template(template_file="usuario/usuario_lista.pt")
async def deshabilitar_usuario(request: Request, id_usuario: int):
    print("Deshabilitar usuario")
    print(f"ID usuario a deshabilitar: {id_usuario}")
    eliminacion = await usuario_service.deshabilitar_usuario(request, id_usuario)
    if not eliminacion["eliminado"]:
        vm = UsuariosViewModel(request)
        await vm.load()
        vm.msg_error = eliminacion["msg_error"]
