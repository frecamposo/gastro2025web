...
# modificacion de archivos para la recupracion de productos desde bodega a taller (aqui)
...
@router.get("/asignatura/deshabilitar/{sigla}")
@template(template_file="asignatura/asignatura_lista.pt")
async def deshabilitar_asignatura(request: Request, sigla: str):
    eliminacion = await asignatura_service.deshabilitar_asignatura(request, sigla)

    if not eliminacion["deshabilitado"]:
        vm = AsignaturasViewModel(request)
        await vm.load()
        vm.msg_error = eliminacion["msg_error"]
...
# permite agregar una observacion al producto del taller
@router.get("/asignatura/{sigla}/taller/{id_taller}/producto/{id_producto}/observacion/{observacion}")
@template(template_file="asignatura/producto_taller_lista.pt")
async def producto_lista(request: Request, sigla: str, id_taller: int, observacion: str, id_producto: int):
    vm = ProductosTallerViewModel(request)
    await vm.update_obs(id_producto, id_taller, observacion)
    await vm.load(sigla=sigla, id_taller=id_taller)
    return vm.to_dict()
