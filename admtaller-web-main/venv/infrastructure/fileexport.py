...
async def generar_excel_taller(datos: List[dict],titulo:str="Reporte Taller"):
    # Crear un DataFrame desde los datos
    df = pd.DataFrame(datos)
    # Esto asume que quieres el título en la primera celda de la nueva fila.
    fila_titulo = pd.DataFrame([titulo], columns=[df.columns[0]])
    # 2. Concatenar la fila del título al inicio del DataFrame principal
    df = pd.concat([df,fila_titulo], ignore_index=True)
    df.iloc[0, 0] = titulo  # Establecer el título en la primera celda
    # 3. Convertir el DataFrame modificado a un archivo Excel
...
async def generar_pdf_taller(datos: List[dict], titulo: str, subtitulo: str = None):
    # Crear el objeto PDF
    pdf = FPDF(orientation='L')
