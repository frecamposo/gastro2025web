...
# Función que carga datos y verifica si está conectado al sistema
    async def update_obs(self, id_producto, id_taller, observacion):
        if self.esta_conectado:
            self.id_taller = id_taller
            self.id_producto = id_producto


            self.productos = await taller_service.update_taller_asignatura_obs(self.id_producto, self.id_taller, observacion)
        else:
            self.msg_error = Mensajes.ERR_NO_AUTENTICADO.value
