...
async def update_taller(request: Request, taller: dict) -> Optional[dict]:
    ...
    print(f"la url es: {url}   y el taller es: {taller}")
    print(f"el json es: {taller}")
    async with httpx.AsyncClient() as client:
        try:
...
async def deshabilitar_asignatura(request: Request, sigla: str) -> Optional[dict]:
    ...
    # Armamos la URL de la API respectiva
    url = f"{APITaller.URL_BASE.value}/asignatura/deshabilitar/{sigla}/{id_usuario}"
